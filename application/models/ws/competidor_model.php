...
class Competidor_model extends CI_Model {
    ...
    function get_competidor_materias($id){
     ...
     function get_competidor_estilos($competidor_id){
        
        $this->db->select('e.*,ce.pontuacao as pontuacao');

        $this->db->from('competidor_estilo_aprendizagem as ce');
        
        $this->db->join('estilo_aprendizagem as e','ce.estilo_aprendizagem_id = e.id');

        $this->db->where('ce.competidor_id', $competidor_id);
        ...
        if ($resultado->num_rows > 0) {
            ...
            $estilos_agrupados_por_dimensao = array();
            
            $estilos = $resultado->result();
            
            foreach($estilos as $estilo) {
                $estilos_agrupados_por_dimensao[$estilo->dimensao][] = $estilo;
            }
            
            return $estilos_agrupados_por_dimensao;
     ...
     * @return boolean Qdo nao encontra nenhum materia com acesso a funcionalidade retorna FALSE
     */
    function get_competidor_problemas($competidor_id){
        
        $this->db->select('problema_id');
    ...
    function insert_estilos_aprendizagem($competidor_id, $pontuacao) {
        
        $inseriu = FALSE;
        
        $estilos = array();
        
        //Se não existe a chave com o estilo é pq o mesmo deve ser zerado
        for($i=1;$i<=8;$i++){
            
            if(!array_key_exists($i, $pontuacao)){
                $pontuacao[$i] = 0;
            }
        }
        
        
        foreach($pontuacao as $estilo => $ponto) {
            
            $estilos = array($competidor_id, $estilo, $ponto, $competidor_id, $estilo, $ponto);
            
            $sql = 'INSERT INTO competidor_estilo_aprendizagem (competidor_id, estilo_aprendizagem_id, pontuacao) VALUES (?,?,?) ON DUPLICATE KEY UPDATE competidor_id = ?, estilo_aprendizagem_id = ?, pontuacao = ?';
            
            $inseriu += (bool)$this->db->query($sql,$estilos);
        }
