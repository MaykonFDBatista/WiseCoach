...
foreach ($filtro['resultado']['dados'] as $usuario) {
    ...
    $this->table->add_row(
            ...
            form_checkbox('selecionados[]', $usuario->id, FALSE, 'class="checkbox icheck" id="check' . $usuario->id . '"'), $usuario->id, anchor($this->config->item('admin') . 'usuario_app/editar/' . $usuario->id, $usuario->nome), $usuario->email, $ativo, date(_lang('formato_data_hora'), strtotime($usuario->data_registro))
    );
}

// Seta a controladora responsavel pelas acoes dos botoes
$dados['controller'] = 'usuario_app';

// Botao da acao controle de acesso
$btn_importar = array('url' => $this->config->item('admin') . $dados['controller'] . '/importacao/',
    'label' => _lang('form_importar'),
    'atributos' => array('class' => 'btn btn-success btn-importacao', 'id' => 'btn-Importar')
);

// Acoes extras que deve ser exibidas na view de gerenciamento
$dados['acoes'] = array($btn_importar);
