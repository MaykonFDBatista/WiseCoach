...
foreach ($filtro['resultado']['dados'] as $regra) {
    
    $this->table->add_row(
            // Checkbox que permite selecionar o registro para que seja aplicada alguma acao sobre ele
            form_checkbox('selecionados[]', $regra->id, FALSE, 'class="checkbox icheck" id="check' . $regra->id . '"'), 
            $regra->id,
            // Transforma o campo nome em um link para edicao do registro
            anchor($this->config->item('admin') . 'regra_pontuacao/editar/' . $regra->id, _lang($regra->descricao)), 
            $regra->pontos,
            date(_lang('formato_data_hora'), strtotime($regra->data_registro))
    );
}

// Seta a controladora responsavel pelas acoes dos botoes
$dados['controller'] = 'regra_pontuacao';
